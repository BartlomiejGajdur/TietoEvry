...
TEST_F(UnitsUnderTestFixture, MoveBase_CannotMove_ReturnsFalse) {
    ...
    bool result = mapa.performMoveByPlayerBelongsToEnemy(mapa.getPlayerBelongsToEnemy()->getUnits()[0]->getId(), validCoord);

    EXPECT_FALSE(result);
    EXPECT_EQ(mapa.getPlayerBelongsToEnemy()->getUnits()[0]->getObjectCoordinates(), Coordinates(3, 4));  // Check that the unit's position remains unchanged
}
